...
import medbot.channels.dingtalk as dingtalk_module
from medbot.channels.dingtalk import DingTalkChannel, NanobotDingTalkHandler
from medbot.channels.dingtalk import DingTalkConfig
...
async def test_handler_processes_file_message(monkeypatch) -> None:
    ...
    async def fake_download(download_code, filename, sender_id):
        return f"/tmp/medbot_dingtalk/{sender_id}/{filename}"
    ...
    assert "/tmp/medbot_dingtalk/user1/report.xlsx" in msg.content
...
async def test_download_dingtalk_file(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "medbot.config.paths.get_media_dir",
