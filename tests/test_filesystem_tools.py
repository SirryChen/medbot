...
from medbot.agent.tools.filesystem import (
    EditFileTool,
    ListDirTool,
    ReadFileTool,
    _find_match,
...
def _minimal_pdf_bytes(text: str) -> bytes:
    ...
    objects = [
        ...
        f"<< /Length {len(f'BT /F1 12 Tf 36 100 Td ({escaped}) Tj ET'.encode('latin-1'))} >>\nstream\nBT /F1 12 Tf 36 100 Td ({escaped}) Tj ET\nendstream".encode("latin-1"),
    ]
...
class TestReadFileTool:
    ...
    @pytest.mark.asyncio
    async def test_pdf_text_is_extracted(self, tool, tmp_path):
        pytest.importorskip("pypdf")
        f = tmp_path / "report.pdf"
        f.write_bytes(_minimal_pdf_bytes("Glucose 6.2 mmol/L"))

        result = await tool.execute(path=str(f))

        assert "[Page 1]" in result
        assert "Glucose 6.2 mmol/L" in result
...
class TestWorkspaceRestriction:
    ...
    async def test_extra_dirs_does_not_widen_write(self, tmp_path):
        from medbot.agent.tools.filesystem import WriteFileTool
