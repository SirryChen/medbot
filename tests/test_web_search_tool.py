...
from medbot.agent.tools.web import WebSearchTool
from medbot.config.schema import WebSearchConfig
...
async def test_brave_search(monkeypatch):
    ...
    result = await tool.execute(query="medbot", count=1)
...
async def test_duckduckgo_search(monkeypatch):
    ...
    monkeypatch.setattr("medbot.agent.tools.web.DDGS", MockDDGS, raising=False)
    import medbot.agent.tools.web as web_mod
