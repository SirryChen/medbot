...
For MedBot, custom skills should live under the active workspace `skills/` directory so they can be discovered automatically at runtime (for example, `<workspace>/skills/my-skill/SKILL.md`).
...
Before finalizing the design, read `skill-filter` and self-review the skill content against it. Remove or narrow any risky, over-broad, deceptive, or policy-bypassing instructions.
...
Keep frontmatter minimal. In MedBot, `metadata` and `always` are also supported when needed, but avoid adding extra fields unless they are actually required.
...
Before packaging, run a final `skill-filter` review on:

- `SKILL.md`
- `scripts/`
- `references/`
- `assets/` if they contain executable or prompt-like content

If the result is `warn`, fix or explicitly surface the concern. If the result is `block`, do not package the skill until the harmful content is removed.
