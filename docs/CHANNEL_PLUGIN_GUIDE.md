...
Build a custom MedBot channel in three steps: subclass, package, install.

## How It Works

MedBot discovers channel plugins via Python [entry points](https://packaging.python.org/en/latest/specifications/entry-points/). When `medbot gateway` starts, it scans:

1. Built-in channels in `medbot/channels/`
2. External packages registered under the `medbot.channels` entry point group
...
```
medbot-channel-webhook/
├── medbot_channel_webhook/
...
```python
# medbot_channel_webhook/__init__.py
from medbot_channel_webhook.channel import WebhookChannel

__all__ = ["WebhookChannel"]
```

```python
# medbot_channel_webhook/channel.py
import asyncio
...
# pyproject.toml
[project]
name = "medbot-channel-webhook"
version = "0.1.0"
dependencies = ["medbot", "aiohttp"]

[project.entry-points."medbot.channels"]
webhook = "medbot_channel_webhook:WebhookChannel"
...
```bash
pip install -e .
medbot plugins list      # verify "Webhook" shows as "plugin"
medbot onboard           # auto-adds default config for detected plugins
```

Edit `~/.medbot/config.json`:
...
| `default_config()` (classmethod) | Returns default config dict for `medbot onboard`. Override to declare your fields. |
...
Override `default_config()` so `medbot onboard` auto-populates `config.json`:
...
| What | Format | Example |
|------|--------|---------|
| PyPI package | `medbot-channel-{name}` | `medbot-channel-webhook` |
| Entry point key | `{name}` | `webhook` |
| Config section | `channels.{name}` | `channels.webhook` |
| Python package | `medbot_channel_{name}` | `medbot_channel_webhook` |

## Local Development

```bash
git clone https://github.com/you/medbot-channel-webhook
cd medbot-channel-webhook
pip install -e .
medbot plugins list    # should show "Webhook" as "plugin"
medbot gateway         # test end-to-end
```
