...
import medbot.channels.matrix as matrix_module
from medbot.bus.events import OutboundMessage
from medbot.bus.queue import MessageBus
from medbot.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from medbot.channels.matrix import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("medbot.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "medbot.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("medbot.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "medbot.channels.matrix.asyncio.create_task", _fake_create_task
    )
